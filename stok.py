...
def stok_menu():
    def stok_ekle_cikart(args):
        ekle = int(gir4.get())
        a=combo.get()
        im.execute("""SELECT urun_adi,adet FROM stok""")
        ...
        if args == 1:
            ...
            son_stok = mevcut_stok-ekle
        im.execute("update stok set adet='{}' WHERE urun_adi='{}'".format(son_stok,a))
        vt.commit()
        gir4.delete(0, "end")
    ...
    for i in pencere.winfo_children():
        i.destroy()
    pencere.title("STOK")
    stok_ekle = Button(text="ÜRÜN EKLE",command=lambda:stok_ekle_cikart(1)).place(x=20, y=20, width=180, height=30)
    stok_cikart = Button(text="ÜRÜN ÇIKART",command=lambda:stok_ekle_cikart(2)).place(x=20, y=60, width=180, height=30)
